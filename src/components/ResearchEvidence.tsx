    ...
    <div className="data-section">
      ...
      <div className="source-plan-grid researched-fact-grid">
        ...
        <article className="intel-card">
          ...
          <span className="evidence-line">Useful for tax context. 0% weight in the ATLAS market estimate.</span>
        </article>
...
export function ResearchCostsSection({ profile, countyRecord }: { profile: ResearchProfile; countyRecord: CountyPropertyRecord | null }) {
  ...
  return (
    <motion.div className="data-section" {...atlasReveal}>
      <div className="section-heading">
        <div><p className="eyebrow">TAXES & COSTS</p><h2>Understand the record first. Then build your budget.</h2></div>
        <p>ATLAS separates what the public record says from what a future owner may actually pay. Blank costs stay excluded instead of being silently guessed.</p>
      </div>

      <motion.aside className="tax-disclaimer-card" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: atlasMotion.duration.reveal, ease: atlasMotion.easing.enter }}>
        <span className="card-kicker">IMPORTANT TAX CONTEXT</span>
        <h3>{tax.taxYear ? `${tax.taxYear} public-record tax` : 'Public-record tax'}: {tax.annualTaxDisplay ?? money(tax.annualTaxPreferred)}</h3>
        <p>This is a corroborated record/reference amount, not a quote of future taxes. A transfer, county reappraisal, new levies, exemptions, special assessments, CAUV or other agricultural treatment, and payment/escrow timing can change the amount billed to an owner.</p>
        <small>For a purchase decision, verify the current parcel tax record and ask how ownership or use changes may affect future tax treatment.</small>
      </motion.aside>

      <motion.section className="cost-planner" layout>
        <div className="cost-total"><span>MONTHLY PLAN SO FAR</span><strong>{money(monthlyTotal)}</strong><small>Uses {money(monthlyTax)}/month only as a planning equivalent of the recorded annual tax—not as a prediction of a future escrow or tax bill.</small></div>
      ...
      <div className="cost-fact-grid">
        <motion.article whileHover={{ y: -3 }} transition={atlasMotion.spring.responsive}><span>{tax.taxYear ?? 'RECENT'} PUBLIC-RECORD TAX</span><strong>{tax.annualTaxDisplay ?? money(tax.annualTaxPreferred)}</strong><p>{tax.sourceCount ?? 'Multiple'} public sources corroborate this recorded amount. It is not a future-tax estimate.</p></motion.article>
        <motion.article whileHover={{ y: -3 }} transition={atlasMotion.spring.responsive}><span>PLANNING EQUIVALENT</span><strong>{money(monthlyTax)}</strong><p>Recorded annual tax ÷ 12. Your actual tax installments or lender escrow can differ.</p></motion.article>
        <motion.article whileHover={{ y: -3 }} transition={atlasMotion.spring.responsive}><span>CAUV / AGRICULTURAL TREATMENT</span><strong>{countyRecord ? (countyRecord.hasCauv ? 'Record indicates enrollment' : 'No enrollment shown') : 'Not verified'}</strong><p>Enrollment, continued eligibility and any recoupment exposure require county-level verification.</p></motion.article>
      </div>
